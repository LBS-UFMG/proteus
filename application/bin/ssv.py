...
def show_mutations(clusters,pdb):
	...
	# BUG residuo errado - Getting original residue
	#lines_r1 = open(pdb).readlines()
	#line_r1 = lines_r1[5]
	#lines_r2 = open(pdb).readlines()
	#line_r2 = lines_r2[17]

	# removing bug - folder address
	aux1 = pdb.split("/")
	aux2 = aux1[-1]
	pos = aux2.replace(".pdb","").split("_")
	r1_pos = pos[0]
	r2_pos = pos[1]

	lines = open(pdb).readlines()
	for line in lines:
		if line[22:26].strip() == r1_pos:
			r1_name = line[17:20].strip()
			continue
		elif line[22:26].strip() == r2_pos:
			r2_name = line[17:20].strip()
			break
...
def send_email(email, project):
	os.system("php ../../application/bin/email.php "+email+" "+project)
